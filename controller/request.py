...
from config import DEFAULT_TOP_K_READER, DEFAULT_TOP_K_RETRIEVER
...
class Question(BaseModel):
    ...
    @classmethod
    def from_elastic_query_dsl(
        cls, query_request: Dict[str, Any], top_k_reader: int = DEFAULT_TOP_K_READER
    ):
        ...
        top_k_retriever: int = (
            ...
            if "size" not in query_request
            else query_request["size"]
        )
    ...
    def _iterate_dsl_request(
        ...
        if depth == MAX_RECURSION_DEPTH:
            raise RecursionError(
                "Parsing incoming DSL reaching current value of the recursion limit"
            )
